...
void PAPlayer::CloseAllStreams(bool fade/* = true */)
{
  if (!fade)
  {
    ...
    while (!m_streams.empty())
    {
      ...
      if(m_bAudio2 && si->m_stream2)
      {
        CServiceBroker::GetActiveAE(m_bAudio2)->FreeStream(si->m_stream2, true);
        si->m_stream2 = NULL;
      }
...
bool PAPlayer::QueueNextFileEx(const CFileItem &file, bool fadeIn)
{
  ...
  if (m_bAudio2)
  {
    ...
    else if (si->m_decoder2.Create(file, si->m_fileItem.m_lStartOffset))
  ...
  {
    double abtm = CUtil::ConvertMilliSecsToSecs(si->m_fileItem.GetProperty("audiobook_bookmark").asInteger());
    si->m_seekFrame = si->m_audioFormat.m_sampleRate * abtm;
    si->m_seekFrame2 = si->m_audioFormat2.m_sampleRate * abtm;
  }
  else
  {
    si->m_seekFrame = -1;
  ...
  if (m_currentStream && ((m_currentStream->m_audioFormat.m_dataFormat == AE_FMT_RAW) || (si->m_audioFormat.m_dataFormat == AE_FMT_RAW) || (m_bAudio2 && ((m_currentStream->m_audioFormat2.m_dataFormat == AE_FMT_RAW) || (si->m_audioFormat2.m_dataFormat == AE_FMT_RAW)))))
  {
...
inline bool PAPlayer::PrepareStream(StreamInfo *si)
{
  ...
  if(m_bAudio2)
  {
    ...
    si->m_stream2 = CServiceBroker::GetActiveAE(m_bAudio2)->MakeStream(
      format,
      AESTREAM_PAUSED
    );
    ...
    else
    {
      ...
      if (peak * gain <= 1.0)
        ...
        // Normalise volume reducing replaygain to avoid needing clipping protection, plays file at lower level
        si->m_stream2->SetReplayGain(1.0f / fabs(peak));
      else
        // Clipping protection (when enabled in AE) by audio limiting, applied just where needed
    ...
    if (m_bAudio2)
    {
      ...
      else
      {
        ...
        if (si->m_audioFormat2.m_dataFormat != AE_FMT_RAW)
          ...
          free_space = (double)si->m_stream2->GetSpace() *
                       si->m_audioFormat2.m_streamInfo.GetDuration(true) / 1000;
...
bool PAPlayer::QueueData(StreamInfo *si)
{
  ...
  if (bAudio2)
  {
    unsigned int space2 = si->m_stream->GetSpace();
    if(space2)
      space = std::min(space, space2);
  }
