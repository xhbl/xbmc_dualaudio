...
bool PAPlayer::QueueNextFileEx(const CFileItem &file, bool fadeIn)
{
  ...
  if (m_bAudio2)
  {
    ...
    else if (si->m_decoder2.Create(file, si->m_startOffset))
  ...
  if (si->m_fileItem->HasProperty("StartPercent") &&
    ...
    sptm = CUtil::ConvertMilliSecsToSecs(static_cast<int>(+(static_cast<double>(
    ...
    sptm = CUtil::ConvertMilliSecsToSecs(
               si->m_fileItem->GetProperty("audiobook_bookmark").asInteger());
  if (sptm > 0)
  {
    si->m_seekFrame = si->m_audioFormat.m_sampleRate * sptm;
    si->m_seekFrame2 = si->m_audioFormat2.m_sampleRate * sptm;
  }
...
inline bool PAPlayer::PrepareStream(StreamInfo *si)
{
  ...
  if (m_currentStream && m_currentStream != si && !m_upcomingCrossfadeMS)
  {
    ...
    if(m_bAudio2)
      m_currentStream->m_stream2->RegisterSlave(si->m_stream2.get());
  }
